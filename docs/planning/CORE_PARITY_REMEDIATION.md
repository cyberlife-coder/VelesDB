...
- **3c — multi-column `ORDER BY` (DONE).** `ORDER BY <lead>, <more…>` (2+ plain `Field` keys, no WHERE) on a
  ...
  fuzz, no divergence).
- **3d — secondary-index persistence.** Split into two layers (an impact audit found the literal BTree
  snapshot concentrates the durability risk for only an O(n)-open-scan saving, so the layers ship separately):
  - **Layer A — config-authority + restore-on-open (DONE).** `CollectionConfig.indexed_fields`
    (`#[serde(default, skip_serializing_if)]`, no schema bump — additive, ignored by older binaries) is the
    persisted **authority**: `create_index` records the field + `save_config` (propagates a persist failure),
    `drop_secondary_index` removes it + best-effort `save_config`. `Collection::open` then rebuilds each listed
    index from the recovered payloads (`build_and_backfill_secondary_index`, after `reconcile_point_count`) —
    provably equal to the live tree (buckets sorted on read, so internal order is irrelevant), including
    TTL-expired-but-unswept rows. Closes the real gap: before this, a `CREATE INDEX` was lost on restart and
    the ordered-index fast path / bitmap pre-filter / `EXPLAIN IndexLookup` / advisor silently changed
