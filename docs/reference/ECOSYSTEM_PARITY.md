...
- **Agent Memory (WASM / Mobile)**: WASM now ships the full high-level `MemoryService` wedge (remember/recall/recallWhere/recallFused/relate/forget/why — in-memory only, no persistence under WASM, #1310) alongside the primitive `SemanticMemory`; mobile remains ⚠️ semantic-only (`VelesSemanticMemory`). Episodic/procedural memory, TTL helpers, and snapshots are not exposed on these bindings.
