...
| `.bench <name> [n] [k]` | Benchmark N random queries with top-k (default: 100 queries, k=10), at the session's search quality (its `ef_search` when set, else its `mode`, else the configured default), which it prints. A failed query stops it with the error. Also available as `\bench`. |
