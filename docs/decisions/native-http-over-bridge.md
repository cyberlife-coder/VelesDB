...
immediately, but `mcp-remote@0.1.38` neither completes the call nor
re-initialises, so the host waits for its own timeout instead. A fresh native
session runs initialize, list and recall with no such wait (no recorded run
times either path).
