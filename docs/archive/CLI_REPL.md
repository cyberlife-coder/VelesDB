...
> **Archive notice:** superseded and largely inaccurate. Five of the nine CLI
> subcommands described here no longer exist under these names (`list`,
> `create`, `import`, `export` now live under `collection` and `data`; `config`
> was removed), the `Rust Implementation` section describes types the crate
> never had, and several `\set` keys documented below are silently ignored.
> Kept for the record only.
>
> For the current surface, see
> [CLI command reference](../guides/CLI_COMMAND_REFERENCE.md),
> [REPL reference](../guides/CLI_REPL_REFERENCE.md),
> [VelesQL cookbook](../guides/CLI_VELESQL_COOKBOOK.md) and
