...
# This plan modifies module-level attributes in lib.rs (lines 61-65: global #[allow] attributes)
# Changes focus on: removing #![allow(...)] at top of file, adding function-level allows
# Non-overlapping with Plan 01-01 (numeric operations/cast sites) and 01-03 (logging statements)
...
grep -c "#\[allow" crates/velesdb-core/src/lib.rs
# Should show function-level allows count

# Verify SAFETY-style justification comments precede each #[allow]
ALLOW_COUNT=$(grep -c "#\[allow" crates/velesdb-core/src/lib.rs)
JUSTIFIED_COUNT=$(grep -B2 "#\[allow" crates/velesdb-core/src/lib.rs | grep -c "SAFETY\|Reason:")
# JUSTIFIED_COUNT should be >= ALLOW_COUNT (some allows may span multiple lines)
echo "Allows: $ALLOW_COUNT, Justified: $JUSTIFIED_COUNT"
test $JUSTIFIED_COUNT -ge $ALLOW_COUNT
```
  </verify>
  <done>All warnings addressed; zero global allows; function-level allows have SAFETY-style justification comments; clippy passes with -D warnings</done>
</task>
