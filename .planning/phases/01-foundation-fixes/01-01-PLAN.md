...
# This plan modifies numeric operations in lib.rs (cast sites like `index as u32`)
# Changes focus on: variable assignments, arithmetic expressions, type conversions
# Non-overlapping with Plan 01-02 (module-level attributes) and 01-03 (logging statements)
# All 3 plans can execute in parallel - changes are to different regions of lib.rs
</context>
...
Count total cast sites for verification:
```bash
CAST_COUNT=$(grep -c " as " crates/velesdb-core/src/ --include="*.rs" -r)
echo "Total 'as' casts found: $CAST_COUNT" >> .planning/phases/01-foundation-fixes/01-01-audit-report.md
```
  </action>
  <verify>
```bash
# Audit report exists and contains count
test -f .planning/phases/01-foundation-fixes/01-01-audit-report.md && \
grep -c " as " crates/velesdb-core/src/ --include="*.rs" -r | grep -q "[0-9]"
```
Audit report exists with complete list of all `as` casts found
Report includes total cast count for baseline tracking
Report categorizes each cast by fix type and risk level
  </verify>
  <done>Complete audit report created with all cast sites identified, categorized, and total count documented (e.g., "Total 'as' casts found: 47")</done>
</task>
...
**Critical**: Tests MUST exercise the actual production code paths:
- Import and call functions from `velesdb_core` that perform conversions
- Test public API methods that accept usize/u64 parameters
- Verify production error types are returned (Error::Overflow)

Example production integration:
```rust
use velesdb_core::{Collection, Error};

#[test]
fn test_collection_with_oversized_dimension() {
    // This exercises the actual production code path
    let result = Collection::create("test", u32::MAX as usize + 1);
...
# Verify tests exercise production code (check imports)
grep -n "use velesdb_core::" crates/velesdb-core/tests/numeric_casts.rs
# Should show imports from production library

# Verify tests call production functions
grep -c "Collection::\|Error::" crates/velesdb-core/tests/numeric_casts.rs
# Should show positive count
```
  </verify>
  <done>Unit tests created and passing; tests import and exercise actual production code paths; covers all boundary conditions for numeric conversions</done>
</task>
...
**INTEGRATED VERIFICATION** (run after all 3 plans complete):
```bash
...
# Verify all goals met across plans:
grep -c " as " crates/velesdb-core/src/ --include="*.rs" -r  # Should show reduced count
grep -c "^#!\[allow" crates/velesdb-core/src/lib.rs  # Should be 0
grep -c "println!\|eprintln!" crates/velesdb-core/src/lib.rs  # Should be 0
```
</verification>
