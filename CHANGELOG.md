...
- **The no-AI-attribution rule now covers pull request bodies, not just commits
  (CLAUDE.md #5).** Commits were guarded; the body was not, and the body is the
  surface the rule loses on by default. Tooling appends a "Generated by ..."
  footer to the description server-side at publish time, so a pull request
  arrives already violating the rule with every commit in it clean. Measured,
  not theorised: three pull requests in one session were published carrying it
  and had to be stripped by hand, and two issue comments still carry it.

  `check-ai-attribution.py` gains `--pr-body`, wired into the existing
  `Reject AI-attributed commits` job as a second step. It deliberately reuses
  `message_is_refused` — the same pattern list a commit message goes through.
  A body-specific list would be a second definition of the rule, and #1699 is
  the record of what that costs: the two inline shell copies this guard
  replaced both knew `claude|anthropic` and nothing else, so Codex, Copilot,
  Cursor and unlisted bots walked straight through.

  The body reaches the guard through `env:` and then a **file**, never as a
  shell argument or a `${{ }}` interpolation inside `run:`. A pull request
  description is attacker-controlled text and interpolating it into a workflow
  script is a command-injection vector.

  Two limits are recorded in the guard's registered blind spot rather than left
  to be discovered. Issue and PR **comments** are still uncovered — no workflow
  trigger delivers a comment body to this guard today, so a footer pasted into
  a comment is caught only by review. And the URL pattern is unanchored, unlike
  the trailer patterns, so a body quoting the footer in order to *describe* the
  rule is refused alongside one that carries it; write the URL in parts when
  documenting it, as this repository's own tests now do.
