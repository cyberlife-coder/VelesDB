...
| Mutation testing | The changed `velesdb-core` code is mutated and the surviving mutants are reported, as far as a 45-minute budget reaches — not a verdict on the whole diff: the one run against a real `velesdb-core` diff (22 mutants, 829 s of unmutated baseline before the first of them) was cut by that budget having reported none. The report is uploaded as an artifact either way | `cargo mutants --in-diff`, `mutants` job in `core-review.yml` (same PRs) |
