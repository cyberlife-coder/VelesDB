...
| Minimal dependency versions | `velesdb-core` and `velesdb-memory` each compile, resolved alone, with every direct dependency at the lowest version its manifest allows, on the MSRV toolchain, on Linux and macOS (not Windows) | `minimal-versions` job in `quality-deep.yml` (weekly, and on PRs that change a `Cargo.toml`) |
