...
| Mutation testing | The changed `velesdb-core` code is mutated and the surviving mutants are reported, as far as a 45-minute budget reaches — not a verdict on the whole diff: in the one run against a real `velesdb-core` diff (run 34767190150), 829 s went to the unmutated baseline and the budget cut the step with **2 of 22 mutants tested, both caught**, a third still building. The artifact carries that partial report; when the filter selects no mutant at all, `cargo mutants` writes no `mutants.out/` and there is no artifact | `cargo mutants --in-diff`, `mutants` job in `core-review.yml` (same PRs) |
