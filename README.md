...
Where Mem0 and Zep are cloud-coupled orchestrators (several backing services plus AI-model calls — cloud by default — on every memory write), this is **one local binary** — fully offline, zero AI calls to store a memory, and an auditable `why()` evidence trail. On the standard LoCoMo memory test, our fully-local setup answers 56% of questions overall and **55–61% of time-related questions** ("when did X happen?") — level with or above the 55.5% (Mem0) and 49.3% (Zep — a measurement Zep disputes) that the [Mem0 paper's evaluation](https://arxiv.org/abs/2504.19413) reports for that category on powerful cloud AI models, while we run on a model on your own machine. (Overall scores from different labs can't be fairly compared — the same product scores 58 or 79 depending on who runs the test — so we don't print a misleading single-number comparison; [full landscape, method and statistics here](crates/velesdb-memory/BENCHMARK.md).) Pick it when your data can't leave the box.
