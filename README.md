...
|---|---|---|
| Ask `why()` and the memory returns the **evidence trail** behind every answer — which facts it used and how they connect, not just the answer itself. That's a built-in audit trail, exactly what regulations like the [EU AI Act (enforceable Aug 2026)](https://artificialintelligenceact.eu/implementation-timeline/) will ask of AI systems. | With the leading alternatives, **every single memory saved makes 2–3 paid calls to a cloud AI service** — and needs an API key. VelesDB stores memories with **zero AI calls and zero keys**: one small program (~9 MB) on your machine, no extra databases to install or operate. | We publish how often the memory **finds the right information** — measured directly on public test sets, with no AI grader in the loop that could flatter the score. Result: up to **+7 to +29 points** better retrieval than a standard vector search, and [anyone can re-run the tests](crates/velesdb-memory/BENCHMARK.md). |
...
Where Mem0 and Zep are cloud-coupled orchestrators (several backing services plus a paid cloud AI call on every memory write), this is **one local binary** — fully offline, zero AI calls to store a memory, and an auditable `why()` evidence trail. On the standard LoCoMo memory test, our fully-local setup answers 56% of questions overall and **61% of time-related questions** ("when did X happen?") — and that 61% is *higher* than what Mem0 (55.5%) and Zep (49.3%) report for the same category in their own tests, even though they run on powerful cloud AI models and we run on a model on your own machine. (Overall scores from different labs can't be fairly compared — the same product scores 58 or 79 depending on who runs the test — so we don't print a misleading single-number comparison; [full landscape and sources here](crates/velesdb-memory/BENCHMARK.md).) Pick it when your data can't leave the box.
