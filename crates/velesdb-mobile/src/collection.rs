...
/// oriented). Shared by every gated read leaf that drops the payload;
/// `query` keeps it and `multi_query_search_ids` maps `(id, score)` pairs,
/// so those two build their own.
