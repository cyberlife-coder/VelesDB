...
/// The deferred-enrichment queue and background worker behind
/// [`MemoryService::remember`]'s autograph path — split out to keep this
/// file under the crate's 500-NLOC-per-file budget, same pattern as
/// [`fused_recall`]. A child module of `service`, so it shares full access
/// to `MemoryService`'s private fields and methods.
#[path = "autograph.rs"]
mod autograph;
use autograph::AutographQueue;
pub use autograph::AutographWorkerHandle;
