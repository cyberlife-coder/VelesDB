...
impl GraphSchema {
    ...
    /// # Errors
    ///
    /// Returns an error when schema is enabled and the node type is not declared.
    ...
    /// # Errors
    ///
    /// Returns an error when schema is enabled and the edge definition or endpoint
    /// node types are invalid or undeclared.
