...
mod tests {
    ...
    fn test_metrics_latency_buckets() {
        let metrics = MatchMetrics::new();
        metrics.record_success(Duration::from_micros(500), 1, 1); // <1ms
        metrics.record_success(Duration::from_millis(3), 1, 1); // <5ms
        metrics.record_success(Duration::from_millis(50), 1, 1); // <100ms
    ...
    fn test_query_timer_drop_counts_as_failure() {
        let metrics = MatchMetrics::new();
        {
            ...
            // Timer dropped without calling success/failure
        }
        assert_eq!(metrics.failed_queries.load(Ordering::Relaxed), 1);
    }
}
