...
impl NativeHnswIndex {
    ...
    ///
    /// The graph places the vector first; `id` is then mapped to the slot it
    /// was given (#2246). If `id` already existed, its old node becomes a
    /// tombstone, filtered out during search via the reverse mapping.
    ///
    /// # Errors
    ///
    /// Returns an error if the dimension does not match or the graph refuses
    /// the vector; the mappings are untouched in both cases.
    ...
    ///
    /// Every dimension is checked before the graph sees any vector; each id
    /// is then mapped to the slot the graph gave its vector (#2246).
    ///
    /// # Errors
    ///
    /// Returns an error if any dimension does not match or the graph refuses
    /// the batch; the mappings are untouched in both cases.
