//! Shared fixtures, assertions and generic behavior suites for the
//! quantized-precision backend tests (`rabitq_precision_tests`,
//! `sq8_precision_tests`).
//!
//! The `check_*` functions are generic over [`TraversalCodec`], so every
//! contract of the shared state machine (lazy training, install alignment,
//! rerank semantics, fallback guards, recall) is written once and pinned
//! for each codec by a thin `#[test]` wrapper in its own test module.
