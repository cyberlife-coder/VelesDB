...
mod tests {
    ...
    fn test_query_context_cardinality_check() {
        ...
        assert!(ctx.check_cardinality(20).is_err()); // 50 + 40 + 20 = 110 > 100
    }
    ...
    fn test_query_context_memory_check() {
        ...
        assert!(ctx.check_memory(200).is_err()); // 500 + 400 + 200 = 1100 > 1000
    }
    ...
    fn test_rate_limiter() {
        ...
        assert!(limiter.check("client1").is_ok());
        assert!(limiter.check("client1").is_err()); // Exceeds 2 QPS
        assert!(limiter.check("client2").is_ok()); // Different client
    }
    ...
    fn test_circuit_breaker_recovery() {
        let cb = CircuitBreaker::new(1, 0); // Immediate recovery for test
        cb.record_failure();
        assert_eq!(cb.state(), CircuitState::Open);

        // Wait and check again
        std::thread::sleep(std::time::Duration::from_millis(10));
        assert!(cb.check().is_ok()); // Should transition to half-open
