...
// `lib.rs`'s `invoke_handler` registration is the single source of truth for
// which commands exist: the tests below parse it (and `build.rs` / the
// permission TOML), so registering a command without declaring its permission
// — or leaving a stale permission behind — fails the suite.
//
// When adding a new command:
// 1. Add the command function to `commands.rs` or a sibling module
// 2. Register it in `lib.rs` `invoke_handler`
// 3. Add it to the build.rs COMMANDS array (generates the permission file)
// 4. Add "allow-{command-name}" to the default.toml [default] section

/// Returns the slice of `source` between `start` and the next `end` marker.
fn extract_block<'a>(source: &'a str, start: &str, end: &str) -> &'a str {
    let from = source.find(start).expect("start marker not found");
    let block = &source[from..];
    let to = block.find(end).expect("end marker not found");
    &block[..to]
}

/// Extracts the command names actually registered in `lib.rs`'s
/// `invoke_handler` block (the `commands*::name` paths inside the
/// `velesdb_invoke_handler!` invocation) — the ground truth that the
/// permission files must cover.
fn commands_registered_in_lib_rs() -> Vec<String> {
    extract_block(include_str!("lib.rs"), "velesdb_invoke_handler!(", "));")
        .lines()
        .map(str::trim)
...
/// Extracts the command names declared in the `build.rs` COMMANDS array
/// (the source of the autogenerated permission files).
fn commands_declared_in_build_rs() -> Vec<String> {
    extract_block(include_str!("../build.rs"), "const COMMANDS", "];")
        .lines()
        .map(str::trim)
...
/// Drift gate: `build.rs` COMMANDS must mirror the actual `invoke_handler`
/// registration in `lib.rs`, in both directions. A registered command
/// missing from build.rs gets no generated permission file and is
/// unreachable from the webview; a build.rs entry for an unregistered
/// command is a stale permission.
#[test]
fn test_build_rs_commands_match_lib_rs_registration() {
    let registered: std::collections::BTreeSet<String> =
        commands_registered_in_lib_rs().into_iter().collect();
    let declared: std::collections::BTreeSet<String> =
        commands_declared_in_build_rs().into_iter().collect();
    let missing: Vec<&String> = registered.difference(&declared).collect();
    let orphaned: Vec<&String> = declared.difference(&registered).collect();
    assert!(
        missing.is_empty(),
        "Commands registered in lib.rs invoke_handler but missing from build.rs COMMANDS: \
         {missing:?}.\n\
         Without a generated permission, these commands are unreachable from the webview."
    );
    assert!(
        orphaned.is_empty(),
        "Commands in build.rs COMMANDS but not registered in lib.rs invoke_handler: \
         {orphaned:?}.\n\
         Remove the stale entries or register the commands."
    );
}

/// Parses `permissions/default.toml` and returns the `[default]` permission
/// strings.
fn default_permission_strings() -> Vec<String> {
    let parsed: toml::Value = toml::from_str(include_str!("../permissions/default.toml"))
        .expect("Failed to parse default.toml as valid TOML");
    parsed
        .get("default")
...
fn test_all_commands_have_default_permissions_toml_parsed() {
    let permission_strings = default_permission_strings();

    for cmd in commands_registered_in_lib_rs() {
        let expected_permission = format!("allow-{}", cmd.replace('_', "-"));
        assert!(
            permission_strings.contains(&expected_permission),
...
fn test_default_permissions_count_matches_commands() {
    let permissions = default_permission_strings();
